...
class DriverController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        if($request->has('searchqry')){
            ...
            $drivers = $drivers->where(function ($queryBuilder) use ($query) {
                $queryBuilder->where('drivers.nombre', 'LIKE', '%'.$query.'%')
                    ...
                    // Si tiene logs, buscar en ellos
                    ->orWhereHas('latestlog.event', function ($queryBuilder) use ($query) {
                        $queryBuilder->where('evento', 'LIKE', '%'.$query.'%');
                    });
            });
        }
        ...
        if($sortCol==='vehicle' && $sortDir) {
            ...
            // Logs
        } else if($sortCol==='logs' && $sortDir) {
            $drivers->join('driver_logs', function($join) {
            ...
            ->select('drivers.*', 'events.evento as evento')
            ->orderByRaw('ISNULL(evento), evento ' . $sortDir);
    ...
    public function store(Request $request)
    {
        ...
        $driver->driver_logs()->create(['event_id' => 1, 'coordenadas' => '-', 'velocidad' => 0]);

        if($request->has('vehicle_id')){
            $driver->driver_logs()->create([
                'event_id' => 2,
                'coordenadas' => '-33.41265,-70.56614',
                'velocidad' => 0
            ]);
        }

        if($request->has('smartcard_id')){
            $driver->driver_logs()->create([
                'event_id' => 3,
                'coordenadas' => '-33.41265,-70.56614',
                'velocidad' => 0
    ...
    public function update(Request $request, $id)
    {
        ...
        $coordenadas = $driver->latestlog->coordenadas;
        $velocidad = $driver->latestlog->velocidad;
